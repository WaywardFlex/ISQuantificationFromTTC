...
path = '/home/ppmluser/Dokumente/data/TTC_segm_5param_test/test'
...
path = ''
file_name = 'dataset_appended_as_testset.xlsx'
# Load base data without split information
df = pd.read_excel(path + '/' + file_name, header=None, usecols=range(6))
df.columns = ['file_name', 'heart_id', 'label', 'slice_number', 'slice_side', 'mass']

split_column = pd.read_excel(path + '/' + file_name, header=None, usecols=[6])
...
mask_colors = {
    ...
    'remaining': (0, 0, 0)  # green
    }

# Example usage
image_filename = 'placebo_V31D22_4_a_IMG6081_cropped.out.jpg'
image_path = os.path.join(path, image_filename)
row = df[df['file_name'] == image_filename].iloc[0]
...
dls = dblock.dataloaders(data, bs=10, val_shuffle=False)
...
# Usage
show_preds(dls, learn, mask_colors, codes, n=2)
torch.cuda.empty_cache()
...
file_name = 'segm4_testset_metrics_cv3.xlsx'
...
idx = 6   # get an image from the valid dataset 
...
# Example usage:
heart_id = "V01D17"  # Specify the heart ID you're interested in
...
with pd.ExcelWriter('segm4_testset_features.xlsx') as writer:
